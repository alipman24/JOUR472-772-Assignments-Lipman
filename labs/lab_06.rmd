...
A4: My question is how calls compare based on the number of people under 18 years old.

After finding the numbers, every county with at least 3,000,000 people under 18 had three of the top four most calls. The outlier is 21223 which had the most calls but was toward the middle of the total number of people under 18. There is generally not much of a correlation beyond those with zip codes being toward the top or bottom regardless of the under 18 population size.

```{r}
baltimore_joined |> group_by(zip_code) |> summarise(u_18 = (mean(pct_under_18) * mean(population)), calls = n()) |> arrange(desc(u_18))
```
