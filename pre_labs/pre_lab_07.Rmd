...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer: The headline should include median before household income since the range would not make sense without that context.**
...
Is the difference between Calvert and Montgomery significant? **Answer: Yes, the margin of error for Calvert is larger than the entire margin for Montgomery.**

Is the difference between Howard and everyone else significant? **Answer: No, there is no overlap with Howard, showing it is definitively the top county.**
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer:** The average income of areas with over 60% high school enrollment is \$111,448 and the average income of areas with under 40% high school enrollment is \$69,252.
...
**Task** What is another question you could ask of this data? **Answer: One question I have is about how state population changes compare by their first letter.**

```{r}
with_letter <- alldata |> mutate(letter = substr(NAME, 1, 1))
```

```{r}
with_letter |> group_by(letter) |> summarise(by_2000_letter = sum(Population2000), by_2010_letter = sum(Population2010)) |> mutate(change = by_2010_letter - by_2000_letter) |> arrange(desc(change))
```
