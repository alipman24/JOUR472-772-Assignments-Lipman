...
**Answer: Using a new variable is important because it allows for the selected data to be accessed later without having to filter it again every time.**
...
**Answer: The expenditure_date column changed to now read the data in a month, day and year format. It replaced the slashes with dashes to reflect that the strings were converted.**
...
**Answer: The values are the earliest date in the month which is the floor date by month.**
...
**Answer: The code is calculating the percentage of votes that went to Moore.**
...
**Answer: The code is now sorting the counties by the percentage that went to Moore in descending order.**
...
**Answer: This answer is similarly interesting and useful because it can show a trend of where Moore did not perform as well, which is as relevant as where he did perform well.**
...
**Answer: str_to_lower does the opposite of str_to_upper by making all characters lowercase. str_to_sentence does the same thing as str_to_upper except that it only ensures the first character of a sentence is uppercase.**
...
**Answer: The code is searching through the strings for specific words and making a new column with state codes if the words are found in the original column's string.**
...
**Answer: Yes, it makes sense for Maryland to have the most expenses in a Maryland based dataset.**
...
1.  Can the str_detect help find courses offered in a calendar year or an academic year?

2.  Can str_detect make payee_type more specific? (ex. newspaper company instead of just business)

3.  How can general_22 show the percent of voters that did not vote for one candidate?
