...
**Answer: The zip code that has had the most money from the Maryland Tourism Board is 21401. That zip code is located in central Annapolis. It does make sense since Annapolis is the state capital and a big city.**

```{r}
cleaned_md_grants_loans |> filter(str_detect(grantee, 'MARYLAND TOURISM COUNCIL')) |> group_by(zip_code) |> summarise(total_by_zip = sum(amount)) |> arrange(desc(total_by_zip))
```
