...
**Task** Load the five CSV files. Do they have the same column structure? **Answer: No, some of the columns are in different orders depending on the file.**
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer: There are 24 rows now and there should be 24 because of the previous table sizes.**
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer: It says that the tables are being joined by county.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer: The county with the most is Prince George's County.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer: Howard is the top county this time and it rose seven spots compared to the previous table. Prince George's County fell by four spots.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer: It has a dbl datatype.**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer: It now has a character datatype.**
