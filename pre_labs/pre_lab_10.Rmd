...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer: The columns have zip code information like the number itself, the name and the size of it on a map.**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer: The table has the information needed to draw the zipcode since its shape and size are included. Each of them have dimensions that are in scale with one another.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code creates a generic map of Maryland and all of its zip codes.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The output is the download of the package to my R Studio.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The output is another table that has similar dimension information for drawing the counties.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The output is only counties that have a zipcode sarting with 24.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code makes a new map of Maryland that is split by county instead of zip code this time.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The zipcodes are now overlaid on top of the counties.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code has forclosures in March 2023 by zipcode.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The code joined the foreclosure data with the zip code data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The map of Maryland is now colored based on the number of foreclosures with a lighter shade of blue representing more in that zip code.**
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer: The output is just that tidycensus and the api key are now loaded.**
...
library(janitor)
library(tidycensus)
census_api_key("600c6ef1015af00bad002507d7f35a726fcde0b7")
```
...
**Task** Run the following code. Describe the output in the space below. **Answer: The ACS data and the foreclosure CSV are now entered as variables and the format is changed to match the ACS data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The table with the county's map information is included alongside the typical ACS data like population.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The two tables are loaded for the geometric data and the forclosures by date.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The data cleaning removes redundancies like having Maryland listed for each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The cleaned geometric table is now joined with the foreclosure data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The joined table now has population rates to reflect the impact of foreclosures in each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The map now uses the county names on top of their matching region and kept the same blue scale but this time it applies to the rate of foreclosure.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The color key changed to now have different colors for different foreclosure rates.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer: Proximity to D.C. seems to correlate with foreclosure rates and the eastern shore of Maryland also struggled with it.**
