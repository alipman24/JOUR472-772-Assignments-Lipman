...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer:** The extra dataframe is removed by selecting the one with data.
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer:** I would be interested in the time of day or the length of the press release.
...
tags there are in the HTML source? **Answer:** The output suggests that the number of rows matches the number of tags in the HTML.
...
tags. Describe the output: **Answer:** The text inside of the li tag is now extracted and separated by row.
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer:** The same li ouput is saved as a tibble now which makes it easier to sort and perform operations on.
